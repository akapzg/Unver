...
impl Account {
    ...
    /// Post an empty JSON object `{}` to a challenge URL to signal readiness
    /// for DNS-01 validation. This avoids the borrow conflict of
    /// `ChallengeHandle::set_ready()`.
    pub async fn set_challenge_ready(&self, challenge_url: &str) -> Result<(), Error> {
        let body = serde_json::json!({});
        self.inner.post(Some(&body), None, challenge_url).await?;
        Ok(())
    }
